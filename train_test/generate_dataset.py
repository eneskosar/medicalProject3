...
def load_things():
    ...
    background_mask = cv2.imread("data/background_mask.png", cv2.IMREAD_GRAYSCALE) == 255

    for path in sorted(Path("data/things").glob("*.png")):
...
def main():
    width = 512
    height = 512
    num_images = 200_000 if len(sys.argv) < 2 else int(sys.argv[1])
